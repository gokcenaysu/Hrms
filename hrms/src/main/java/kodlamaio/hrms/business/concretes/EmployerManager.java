...
public class EmployerManager implements EmployerService {
	...
	/*
	 * @Override public Result update(Employer employer, int userId) { Employer
	 * employers = getById(userId).getData(); if(employer.getCompanyName().isEmpty()
	 * || employer.getWebsite().isEmpty() || employer.getPhoneNumber().isEmpty() ||
	 * employer.getEmail().isEmpty() || employer.getPassword().isEmpty() ||
	 * employer.getWebsite().isBlank() || employer.getPassword().isBlank()) { return
	 * new ErrorResult("Fields cannot be left blank"); }
	 * 
	 * String email = employer.getEmail(); String[] emailSplit = email.split("@");
	 * if(!emailSplit[1].equals(employer.getWebsite())) { return new
	 * ErrorResult("Your e-mail address and domain do not match"); }
	 * 
	 * if(employerDao.getByEmailEquals(employer.getEmail())!=null) { return new
	 * ErrorResult("Registered e-mail"); }
	 * 
	 * if(!regexService.isPhoneNumberFormat(employer.getPhoneNumber())) { return new
	 * ErrorResult("Invalid phone number"); }
	 * 
	 * if(employerDao.getByWebsiteEquals(employer.getWebsite())!=null) { return new
	 * ErrorResult("Registered website"); }
	 * 
	 * if(employerDao.getByPhoneNumberEquals(employer.getPhoneNumber())!=null) {
	 * return new ErrorResult("Registered phone number"); }
	 * 
	 * else { this.employerDao.save(employer); return new
	 * SuccessResult("Your registration has been created successfully"); } }
	 */
